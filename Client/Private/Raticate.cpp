...
CRaticate::CRaticate(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CRaticate::CRaticate(const CRaticate & rhs)
	: CGameObj(rhs)
{
}

HRESULT CRaticate::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRaticate::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Bite"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
...
void CRaticate::Tick(_float fTimeDelta)
{
...
void CRaticate::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CRaticate::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CRaticate::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CRaticate::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Raticate"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CRaticate::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CRaticate::Move(_float fTimeDelta)
{
...
void CRaticate::Ready_EvolScript()
{
...
void CRaticate::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 꼬렛은  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'레트라'는 깨물어부수기를 배웠다."));
}
void CRaticate::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CRaticate::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CRaticate::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CRaticate::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("레트라");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("레트라");
	m_PokemonInfo.strInfo = TEXT("뻗어나가는 앞니를 없애기 위해 단단한 것을 긁는 습성.\n블록벽도 갉아서 무너뜨린다.");
	m_PokemonInfo.strChar = TEXT("짜증");
	m_PokemonInfo.iPokeNum = 20;
...
HRESULT CRaticate::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CRaticate * CRaticate::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRaticate*	pInstance = new CRaticate(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CRaticate"));
...
CGameObject * CRaticate::Clone(void * pArg)
{
	CGameObject*	pInstance = new CRaticate(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CRaticate"));
		Safe_Release(pInstance);
	}
