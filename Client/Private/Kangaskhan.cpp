...
CKangaskhan::CKangaskhan(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CKangaskhan::CKangaskhan(const CKangaskhan & rhs)
	: CGameObj(rhs)
{
}

HRESULT CKangaskhan::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CKangaskhan::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_MegaPunch"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BodyPress"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_RockSlide"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum4)))
		return E_FAIL;
...
void CKangaskhan::Tick(_float fTimeDelta)
{
...
void CKangaskhan::Late_Tick(_float fTimeDelta)
{
...
HRESULT CKangaskhan::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CKangaskhan::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CKangaskhan::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Kangaskhan"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CKangaskhan::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CKangaskhan::Move(_float fTimeDelta)
{
...
void CKangaskhan::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CKangaskhan::CheckRide()
{
	if (!dynamic_cast<CGameObj*>(m_pTarget)->Get_Ride())
		m_PlayerInfo.bRide = false;
}
void CKangaskhan::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CKangaskhan::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("캥카");
	m_PlayerInfo.bEvent = false;

	m_PokemonInfo.strName = TEXT("캥카");
	m_PokemonInfo.strInfo = TEXT("배의 주머니에서 새끼를 키운다.\n안전할 때만 새끼를 주머니에서 꺼내어 놀게 한다");
	m_PokemonInfo.strChar = TEXT("카와이");
	m_PokemonInfo.iPokeNum = 115;
...
HRESULT CKangaskhan::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CKangaskhan * CKangaskhan::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CKangaskhan*	pInstance = new CKangaskhan(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CKangaskhan"));
...
CGameObject * CKangaskhan::Clone(void * pArg)
{
	CGameObject*	pInstance = new CKangaskhan(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CKangaskhan"));
		Safe_Release(pInstance);
	}
