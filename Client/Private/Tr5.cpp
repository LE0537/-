...
CTr5::CTr5(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr5::CTr5(const CTr5 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr5::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr5::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr5::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr5::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr5::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr5::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr5"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr5::Move(_float fTimeDelta)
{
	...
	vPos += XMVector3Normalize(vLook) * 1.5f * fTimeDelta;
	m_pTransformCom->Set_State(CTransform::STATE_TRANSLATION, vPos);
	m_pTransformCom->LookAt(vTargetPos);
}

void CTr5::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("에구구..."));
	m_vNormalScript.push_back(TEXT("노인을 봤으면 인사부터해야지!"));
	m_vNormalScript.push_back(TEXT("혼쭐을 내줄테다!"));

	wstring szScriptBegin = TEXT("이래뵈도 아직 팔팔한 80대 라구!!\n가라!!     '");
...
void CTr5::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("에구머니나 젊은애가 사람 잡네!!");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("요정의배지를 얻었습니다.\n 5000원을 얻었다."));
}
void CTr5::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr5::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr5::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(5000);
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_FairyBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr5::Battle(_float fTimeDelta)
{
...
HRESULT CTr5::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr5 * CTr5::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr5*	pInstance = new CTr5(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr5"));
...
CGameObject * CTr5::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr5(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr5"));
		Safe_Release(pInstance);
	}
