...
void CThunder3::Late_Tick(_float fTimeDelta)
{
...
HRESULT CThunder3::Render_ShadowDepth()
{
	return E_NOTIMPL;
}
HRESULT CThunder3::Ready_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Thunder3"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
void CThunder3::Set_Pos(_float fTimeDelta)
{
...
HRESULT CThunder3::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CThunder3 * CThunder3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	...
	{
		ERR_MSG(TEXT("Failed to Created : CThunder3"));
...
{
	CGameObject*	pInstance = new CThunder3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CThunder3"));
		Safe_Release(pInstance);
	}
