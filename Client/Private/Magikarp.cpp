...
CMagikarp::CMagikarp(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CMagikarp::CMagikarp(const CMagikarp & rhs)
	: CGameObj(rhs)
{
}

HRESULT CMagikarp::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMagikarp::Initialize(void * pArg)
{
...
void CMagikarp::Tick(_float fTimeDelta)
{
...
void CMagikarp::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CMagikarp::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CMagikarp::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CMagikarp::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Magikarp"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CMagikarp::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CMagikarp::Move(_float fTimeDelta)
{
...
void CMagikarp::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CGyarados*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CGyarados*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CGyarados*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CGyarados*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Gyarados"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CMagikarp::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMagikarp::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.06f, 0.06f, 0.06f, 0.f));
...
void CMagikarp::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("잉어킹");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("잉어킹");
	m_PokemonInfo.strInfo = TEXT("힘도 스피드도 거의 없다.\n세상에서 가장 약하고 한심한 포켓몬이다.");
	m_PokemonInfo.strChar = TEXT("파닥파닥");
	m_PokemonInfo.iPokeNum = 129;
...
HRESULT CMagikarp::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMagikarp * CMagikarp::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMagikarp*	pInstance = new CMagikarp(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMagikarp"));
...
CGameObject * CMagikarp::Clone(void * pArg)
{
	CGameObject*	pInstance = new CMagikarp(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMagikarp"));
		Safe_Release(pInstance);
	}
