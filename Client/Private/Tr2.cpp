...
CTr2::CTr2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr2::CTr2(const CTr2 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr2::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr2::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr2::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr2::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr2::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr2"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr2::Move(_float fTimeDelta)
{
...
void CTr2::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("헛둘!!헛둘!!셋!넷!!"));
	m_vNormalScript.push_back(TEXT("내 길을 막다니...가만 안둬!!"));

	wstring szScriptBegin = TEXT("하하하 사실 난 관장이다!!\n가라!!     '");
...
void CTr2::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("???:다음부턴 이런 일 없도록 할게요...");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("불의배지를 얻었습니다.\n 3000원을 얻었다."));
}
void CTr2::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr2::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr2::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(3000);
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_FireBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr2::Battle(_float fTimeDelta)
{
...
HRESULT CTr2::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr2 * CTr2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr2*	pInstance = new CTr2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr2"));
...
CGameObject * CTr2::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr2"));
		Safe_Release(pInstance);
	}
