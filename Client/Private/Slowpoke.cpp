...
CSlowpoke::CSlowpoke(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CSlowpoke::CSlowpoke(const CSlowpoke & rhs)
	: CGameObj(rhs)
{
}

HRESULT CSlowpoke::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CSlowpoke::Initialize(void * pArg)
{
...
void CSlowpoke::Tick(_float fTimeDelta)
{
...
void CSlowpoke::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CSlowpoke::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CSlowpoke::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CSlowpoke::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Slowpoke"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CSlowpoke::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CSlowpoke::Move(_float fTimeDelta)
{
...
void CSlowpoke::Ready_EvolScript()
{
...
void CSlowpoke::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CSlowbro*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CSlowbro*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CSlowbro*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CSlowbro*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Slowbro"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CSlowpoke::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CSlowpoke::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.06f, 0.06f, 0.06f, 0.f));
...
void CSlowpoke::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("야돈");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("야돈");
	m_PokemonInfo.strInfo = TEXT("항상 멍하니 있지만 가끔 찌릿 하고 인상을 쓴다.\n 먹고 있는 먹이가 원인이라는 듯 하다.");
	m_PokemonInfo.strChar = TEXT("게으름");
	m_PokemonInfo.iPokeNum = 79;
...
HRESULT CSlowpoke::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CSlowpoke * CSlowpoke::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CSlowpoke*	pInstance = new CSlowpoke(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CSlowpoke"));
...
CGameObject * CSlowpoke::Clone(void * pArg)
{
	CGameObject*	pInstance = new CSlowpoke(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CSlowpoke"));
		Safe_Release(pInstance);
	}
