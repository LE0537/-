...
CDandel::CDandel(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CDandel::CDandel(const CDandel & rhs)
	: CGameObj(rhs)
{
}

HRESULT CDandel::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDandel::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
HRESULT CDandel::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Dandel"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CDandel::Move(_float fTimeDelta)
{
...
void CDandel::Ckeck_Dist()
{
	_vector vTargetPos = dynamic_cast<CGameObj*>(m_pTarget)->Get_Transfrom()->Get_State(CTransform::STATE_TRANSLATION);
	_vector vLook = vTargetPos - m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	m_fDist = XMVectorGetX(XMVector3Length(vLook));
}
void CDandel::Check_Coll()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CDandel::Check_Anim(_float fTimeDelta)
{
...
void CDandel::Battle(_float fTimeDelta)
{
...
HRESULT CDandel::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDandel * CDandel::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CDandel*	pInstance = new CDandel(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CDandel"));
...
CGameObject * CDandel::Clone(void * pArg)
{
	CGameObject*	pInstance = new CDandel(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CDandel"));
		Safe_Release(pInstance);
	}
