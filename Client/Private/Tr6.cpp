...
CTr6::CTr6(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr6::CTr6(const CTr6 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr6::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr6::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr6::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr6::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr6::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr6::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr6"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr6::Move(_float fTimeDelta)
{
...
void CTr6::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("yo~hey~"));
	m_vNormalScript.push_back(TEXT("돈까스 좋아해요?"));
	m_vNormalScript.push_back(TEXT("은보?서호?홍주?가츠몽?"));

	wstring szScriptBegin = TEXT("돈!!가!!스!!\n가라!!     '");
...
void CTr6::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr6::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(5500);
...
void CTr6::Battle(_float fTimeDelta)
{
...
HRESULT CTr6::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr6 * CTr6::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr6*	pInstance = new CTr6(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr6"));
...
CGameObject * CTr6::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr6(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr6"));
		Safe_Release(pInstance);
	}
