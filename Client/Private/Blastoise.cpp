...
CBlastoise::CBlastoise(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CBlastoise::CBlastoise(const CBlastoise & rhs)
	: CGameObj(rhs)
{
}

HRESULT CBlastoise::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBlastoise::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_WaterGun"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_WaterBeam"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_HydroPump"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum4)))
		return E_FAIL;
...
void CBlastoise::Tick(_float fTimeDelta)
{
...
void CBlastoise::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CBlastoise::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CBlastoise::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CBlastoise::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Blastoise"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CBlastoise::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CBlastoise::Ready_Script()
{
	wstring szScriptBegin = TEXT("¾Ñ!! ¾ß»ýÀÇ '");
	wstring szScriptEnd = TEXT("'(ÀÌ)°¡   ³ªÅ¸³µ´Ù!");
	szScriptBegin += m_PokemonInfo.strName;
	szScriptBegin += szScriptEnd;
	m_vNormalScript.push_back(szScriptBegin);
}
void CBlastoise::Move(_float fTimeDelta)
{
...
void CBlastoise::Ready_EvolScript()
{
...
void CBlastoise::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("ÃàÇÏÇÕ´Ï´Ù! ¾î´ÏºÎ±â´Â  \n'");
	wstring strTextEnd = TEXT("' ·Î ÁøÈ­Çß½À´Ï´Ù!");

	strTextBegin += m_PokemonInfo.strName;
	strTextBegin += strTextEnd;

	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'°ÅºÏ¿Õ'Àº ÇÏÀÌµå·ÎÆßÇÁ¸¦ ¹è¿ü´Ù."));
}
void CBlastoise::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CBlastoise::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBlastoise::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CBlastoise::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("°ÅºÏ¿Õ");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("°ÅºÏ¿Õ");
	m_PokemonInfo.strInfo = TEXT("µî²®ÁúÀÇ ·ÎÄÏÆ÷¿¡¼­ ³»»Õ´Â ¼ö·ù´Â\nµÎÅùÇÑ Ã¶ÆÇµµ ÇÑ¹ß¿¡ ¶Õ´Â ÆÄ±«·Â.");
	m_PokemonInfo.strChar = TEXT("´ëÀåºÎ");
	m_PokemonInfo.iPokeNum = 9;
...
HRESULT CBlastoise::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CBlastoise * CBlastoise::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBlastoise*	pInstance = new CBlastoise(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CBlastoise"));
...
CGameObject * CBlastoise::Clone(void * pArg)
{
	CGameObject*	pInstance = new CBlastoise(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBlastoise"));
		Safe_Release(pInstance);
	}
