...
CMew::CMew(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CMew::CMew(const CMew & rhs)
	: CGameObj(rhs)
{
}

HRESULT CMew::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMew::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Psychic"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Thunder"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_DragonPulse"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum4)))
		return E_FAIL;
...
void CMew::Tick(_float fTimeDelta)
{
	if (m_bOnOff)
	{
		Set_DeckPos();
...
void CMew::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMew::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CMew::Ready_Components()
{
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Mew"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CMew::Move(_float fTimeDelta)
{
...
void CMew::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMew::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.1f, 0.1f, 0.1f, 0.f));
...
void CMew::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("뮤");
	m_PlayerInfo.bEvent = false;

	m_PokemonInfo.strName = TEXT("뮤");
	m_PokemonInfo.strInfo = TEXT("모든 기술을 사용하기 때문에 \n포켓몬의 조상이라고 생각하는 학자가 많다.");
	m_PokemonInfo.strChar = TEXT("환상");
	m_PokemonInfo.iPokeNum = 151;
...
HRESULT CMew::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMew * CMew::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMew*	pInstance = new CMew(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMew"));
...
CGameObject * CMew::Clone(void * pArg)
{
	CGameObject*	pInstance = new CMew(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMew"));
		Safe_Release(pInstance);
	}
