...
CDratini::CDratini(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CDratini::CDratini(const CDratini & rhs)
	: CGameObj(rhs)
{
}

HRESULT CDratini::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDratini::Initialize(void * pArg)
{
...
void CDratini::Tick(_float fTimeDelta)
{
...
void CDratini::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CDratini::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CDratini::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CDratini::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Dratini"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CDratini::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CDratini::Move(_float fTimeDelta)
{
...
void CDratini::Ready_EvolScript()
{
...
void CDratini::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CDragonair*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CDragonair*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CDragonair*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CDragonair*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Dragonair"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CDratini::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CDratini::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.06f, 0.06f, 0.06f, 0.f));
...
void CDratini::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("미뇽");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("미뇽");
	m_PokemonInfo.strInfo = TEXT("오랫동안 환상이라고 불렸다.\n이마에 있는 작은 돌기는 성장하고 있는 뿔이다");
	m_PokemonInfo.strChar = TEXT("영롱");
	m_PokemonInfo.iPokeNum = 147;
...
void CDratini::LvUp()
{
	...
	RELEASE_INSTANCE(CGameInstance);
	if (m_PokemonInfo.iLv >= 30)
		m_PokemonInfo.bEvolution = true;
	m_PokemonInfo.bLvUp = false;
}
void CDratini::Reset_Battle()
{
	m_iAnim = 0;
	m_bSetPos = false;
	m_bBrath = false;
...
HRESULT CDratini::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDratini * CDratini::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CDratini*	pInstance = new CDratini(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CDratini"));
...
CGameObject * CDratini::Clone(void * pArg)
{
	CGameObject*	pInstance = new CDratini(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CDratini"));
		Safe_Release(pInstance);
	}
