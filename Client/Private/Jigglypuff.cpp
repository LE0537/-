...
CJigglypuff::CJigglypuff(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CJigglypuff::CJigglypuff(const CJigglypuff & rhs)
	: CGameObj(rhs)
{
}

HRESULT CJigglypuff::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CJigglypuff::Initialize(void * pArg)
{
...
void CJigglypuff::Tick(_float fTimeDelta)
{
...
void CJigglypuff::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CJigglypuff::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CJigglypuff::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CJigglypuff::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Jigglypuff"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CJigglypuff::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CJigglypuff::Move(_float fTimeDelta)
{
...
void CJigglypuff::Ready_EvolScript()
{
...
void CJigglypuff::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CWigglytuff*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CWigglytuff*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CWigglytuff*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CWigglytuff*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Wigglytuff"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CJigglypuff::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CJigglypuff::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CJigglypuff::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("푸린");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("푸린");
	m_PokemonInfo.strInfo = TEXT("목소리 파장을 자유자재로 바꾸어 \n들으면 반드시 잠드는 신비한 멜로디를 노래한다.");
	m_PokemonInfo.strChar = TEXT("말괄량이");
	m_PokemonInfo.iPokeNum = 39;
	m_PokemonInfo.iLv = rand() % 5 + 5;
...
void CJigglypuff::LvUp()
{
	...
	RELEASE_INSTANCE(CGameInstance);
	if (m_PokemonInfo.iLv >= 20)
		m_PokemonInfo.bEvolution = true;
	m_PokemonInfo.bLvUp = false;
}
void CJigglypuff::Reset_Battle()
{
	m_iAnim = 0;
	m_bSetPos = false;
	m_bBrath = false;
...
HRESULT CJigglypuff::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CJigglypuff * CJigglypuff::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CJigglypuff*	pInstance = new CJigglypuff(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CJigglypuff"));
...
CGameObject * CJigglypuff::Clone(void * pArg)
{
	CGameObject*	pInstance = new CJigglypuff(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CJigglypuff"));
		Safe_Release(pInstance);
	}
