...
CBallEffect3::CBallEffect3(const CBallEffect3 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CBallEffect3::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBallEffect3::Initialize(void * pArg)
{
...
void CBallEffect3::Tick(_float fTimeDelta)
{
	Set_Pos(fTimeDelta);

}

void CBallEffect3::Late_Tick(_float fTimeDelta)
{
...
void CBallEffect3::Set_Pos(_float fTimeDelta)
{
...
HRESULT CBallEffect3::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CBallEffect3 * CBallEffect3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBallEffect3*	pInstance = new CBallEffect3(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CBallEffect3"));
...
CGameObject * CBallEffect3::Clone(void * pArg)
{
	CGameObject*	pInstance = new CBallEffect3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBallEffect3"));
		Safe_Release(pInstance);
	}
