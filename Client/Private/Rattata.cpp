...
CRattata::CRattata(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CRattata::CRattata(const CRattata & rhs)
	: CGameObj(rhs)
{
}

HRESULT CRattata::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRattata::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_WaterBeam"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
...
void CRattata::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CRattata::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Rattata"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CRattata::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CRattata::Move(_float fTimeDelta)
{
...
void CRattata::Ready_EvolScript()
{
...
void CRattata::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CRaticate*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CRaticate*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CRaticate*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CRaticate*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Raticate"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CRattata::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CRattata::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CRattata::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("²¿·¿");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("²¿·¿");
	m_PokemonInfo.strInfo = TEXT("¾î¶°ÇÑ Àå¼Ò¶óµµ Á¤ÂøÇØ »ì¾Æ°¥ ¼ö ÀÖ´Â »ý¸í·Â.\n °æ°è½ÉÀÌ ¸Å¿ì °­ÇÏ´Ù.");
	m_PokemonInfo.strChar = TEXT("¾å»ðÀÌ");
...
HRESULT CRattata::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CRattata * CRattata::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CRattata*	pInstance = new CRattata(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CRattata"));
...
CGameObject * CRattata::Clone(void * pArg)
{
	CGameObject*	pInstance = new CRattata(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CRattata"));
		Safe_Release(pInstance);
	}
