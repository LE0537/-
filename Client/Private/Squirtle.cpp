...
CSquirtle::CSquirtle(const CSquirtle & rhs)
	: CGameObj(rhs)
{
}

HRESULT CSquirtle::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CSquirtle::Initialize(void * pArg)
{
...
void CSquirtle::Tick(_float fTimeDelta)
{
...
HRESULT CSquirtle::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CSquirtle::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CSquirtle::Ready_Components()
{
	/* For.Com_Renderer */
...
void CSquirtle::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CSquirtle::Move(_float fTimeDelta)
{
...
void CSquirtle::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CSquirtle::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.085f, 0.085f, 0.085f, 0.f));
...
void CSquirtle::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("²¿ºÎ±â");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("²¿ºÎ±â");
...
CSquirtle * CSquirtle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CSquirtle*	pInstance = new CSquirtle(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CSquirtle"));
...
CGameObject * CSquirtle::Clone(void * pArg)
{
	CGameObject*	pInstance = new CSquirtle(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CSquirtle"));
		Safe_Release(pInstance);
	}
