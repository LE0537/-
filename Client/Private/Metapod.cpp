...
CMetapod::CMetapod(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CMetapod::CMetapod(const CMetapod & rhs)
	: CGameObj(rhs)
{
}

HRESULT CMetapod::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMetapod::Initialize(void * pArg)
{
...
void CMetapod::Tick(_float fTimeDelta)
{
...
void CMetapod::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CMetapod::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CMetapod::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CMetapod::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Metapod"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CMetapod::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CMetapod::Move(_float fTimeDelta)
{
...
void CMetapod::Ready_EvolScript()
{
...
void CMetapod::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 캐터피는  \n'");
...
void CMetapod::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CMetapod::Set_EvolPos2(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Butterfree"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
		_int iIndex = dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_EvolIndex();
		dynamic_cast<CGameObj*>(m_EvolPoke)->Set_Target(m_pTarget);
...
void CMetapod::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMetapod::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CMetapod::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("단데기");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("단데기");
	m_PokemonInfo.strInfo = TEXT("진화를 기다리는 상태다.\n단단해지는 것밖에 할 수 없어서\n공격받지 않도록 꿈쩍 않고 있다.");
	m_PokemonInfo.strChar = TEXT("조심");
	m_PokemonInfo.iPokeNum = 11;
...
HRESULT CMetapod::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMetapod * CMetapod::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMetapod*	pInstance = new CMetapod(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMetapod"));
...
CGameObject * CMetapod::Clone(void * pArg)
{
	CGameObject*	pInstance = new CMetapod(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMetapod"));
		Safe_Release(pInstance);
	}
