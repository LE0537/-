...
CScyther::CScyther(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CScyther::CScyther(const CScyther & rhs)
	: CGameObj(rhs)
{
}

HRESULT CScyther::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CScyther::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Scissor"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BugBuzz"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
...
void CScyther::Tick(_float fTimeDelta)
{
...
void CScyther::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CScyther::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CScyther::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CScyther::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Scyther"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CScyther::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CScyther::Move(_float fTimeDelta)
{
...
void CScyther::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CScyther::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.055f, 0.055f, 0.055f, 0.f));
...
void CScyther::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("스라크");
	m_PlayerInfo.bEvent = false;

	m_PokemonInfo.strName = TEXT("스라크");
	m_PokemonInfo.strInfo = TEXT("싸움을 거듭하면서 낫은 점점 더 날카로워진다.\n거목도 일도양단으로 베어 가른다.");
	m_PokemonInfo.strChar = TEXT("사나움");
	m_PokemonInfo.iPokeNum = 123;
...
HRESULT CScyther::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CScyther * CScyther::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CScyther*	pInstance = new CScyther(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CScyther"));
...
CGameObject * CScyther::Clone(void * pArg)
{
	CGameObject*	pInstance = new CScyther(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CScyther"));
		Safe_Release(pInstance);
	}
