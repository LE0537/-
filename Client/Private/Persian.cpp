...
CPersian::CPersian(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CPersian::CPersian(const CPersian & rhs)
	: CGameObj(rhs)
{
}

HRESULT CPersian::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CPersian::Initialize(void * pArg)
{
...
void CPersian::Tick(_float fTimeDelta)
{
...
void CPersian::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CPersian::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CPersian::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CPersian::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Persian"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CPersian::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CPersian::Move(_float fTimeDelta)
{
...
void CPersian::Ready_EvolScript()
{
...
void CPersian::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 나옹은  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'페르시온'은 깨물어부수기를 배웠다."));
}
void CPersian::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CPersian::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CPersian::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.06f, 0.06f, 0.06f, 0.f));
...
void CPersian::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("페르시온");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("페르시온");
	m_PokemonInfo.strInfo = TEXT("자존심이 세서 길들이기 힘들다.\n 마음에 들지 않는 일이 있으면 바로 발톱을 세운다.");
	m_PokemonInfo.strChar = TEXT("강렬");
	m_PokemonInfo.iPokeNum = 53;
...
HRESULT CPersian::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CPersian * CPersian::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CPersian*	pInstance = new CPersian(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CPersian"));
...
CGameObject * CPersian::Clone(void * pArg)
{
	CGameObject*	pInstance = new CPersian(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CPersian"));
		Safe_Release(pInstance);
	}
