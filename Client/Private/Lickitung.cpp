...
CLickitung::CLickitung(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CLickitung::CLickitung(const CLickitung & rhs)
	: CGameObj(rhs)
{
}

HRESULT CLickitung::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CLickitung::Initialize(void * pArg)
{
...
void CLickitung::Tick(_float fTimeDelta)
{
...
void CLickitung::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CLickitung::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CLickitung::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CLickitung::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Lickitung"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CLickitung::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CLickitung::Move(_float fTimeDelta)
{
...
void CLickitung::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CLickitung::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.065f, 0.065f, 0.065f, 0.f));
...
void CLickitung::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("내루미");
	m_PlayerInfo.bEvent = false;

	m_PokemonInfo.strName = TEXT("내루미");
...
HRESULT CLickitung::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CLickitung * CLickitung::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CLickitung*	pInstance = new CLickitung(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CLickitung"));
...
CGameObject * CLickitung::Clone(void * pArg)
{
	CGameObject*	pInstance = new CLickitung(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CLickitung"));
		Safe_Release(pInstance);
	}
