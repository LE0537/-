...
CBallRed2::CBallRed2(const CBallRed2 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CBallRed2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBallRed2::Initialize(void * pArg)
{
...
void CBallRed2::Tick(_float fTimeDelta)
{
	Set_Pos(fTimeDelta);

}

void CBallRed2::Late_Tick(_float fTimeDelta)
{
...
void CBallRed2::Set_Pos(_float fTimeDelta)
{
	m_fSize -= 15.f * fTimeDelta;
...
HRESULT CBallRed2::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CBallRed2 * CBallRed2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBallRed2*	pInstance = new CBallRed2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CBallRed2"));
...
CGameObject * CBallRed2::Clone(void * pArg)
{
	CGameObject*	pInstance = new CBallRed2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBallRed2"));
		Safe_Release(pInstance);
	}
