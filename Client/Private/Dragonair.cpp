...
CDragonair::CDragonair(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CDragonair::CDragonair(const CDragonair & rhs)
	: CGameObj(rhs)
{
}

HRESULT CDragonair::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CDragonair::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_DragonPulse"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
...
void CDragonair::Tick(_float fTimeDelta)
{
...
void CDragonair::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CDragonair::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CDragonair::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CDragonair::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Dragonair"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CDragonair::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CDragonair::Move(_float fTimeDelta)
{
...
void CDragonair::Ready_EvolScript()
{
...
void CDragonair::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 미뇽은  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'신뇽'은 용의파동을 배웠다."));
}
void CDragonair::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_PokemonInfo.bEvolution)
	{
		if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Dragonite"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
				return;
...
void CDragonair::Set_EvolPos2(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CDragonite*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CDragonite*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		dynamic_cast<CDragonite*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CDragonite*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
...
void CDragonair::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CDragonair::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.05f, 0.05f, 0.05f, 0.f));
...
void CDragonair::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("신뇽");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("신뇽");
...
HRESULT CDragonair::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDragonair * CDragonair::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CDragonair*	pInstance = new CDragonair(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CDragonair"));
...
CGameObject * CDragonair::Clone(void * pArg)
{
	CGameObject*	pInstance = new CDragonair(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CDragonair"));
		Safe_Release(pInstance);
	}
