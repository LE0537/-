...
CCaterpie::CCaterpie(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CCaterpie::CCaterpie(const CCaterpie & rhs)
	: CGameObj(rhs)
{
}

HRESULT CCaterpie::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CCaterpie::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_WaterGun"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
...
void CCaterpie::Tick(_float fTimeDelta)
{
...
void CCaterpie::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CCaterpie::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Caterpie"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CCaterpie::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CCaterpie::Move(_float fTimeDelta)
{
...
void CCaterpie::Ready_EvolScript()
{
...
void CCaterpie::Set_DeckPos()
{
	...
	if (m_bDeckInfo)
	{
		...
		_float3 vScale = { m_fSizeX,m_fSizeY,30.f };
...
void CCaterpie::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Metapod"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CCaterpie::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CCaterpie::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.15f, 0.15f, 0.15f, 0.f));
...
void CCaterpie::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("캐터피");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("캐터피");
	m_PokemonInfo.strInfo = TEXT("다리는 짧지만 빨판으로 되어있어\n비탈길이나 벽에서도 지치지 않고 나아간다.");
	m_PokemonInfo.strChar = TEXT("소심");
	m_PokemonInfo.iPokeNum = 10;
...
void CCaterpie::LvUp()
{
	...
	RELEASE_INSTANCE(CGameInstance);
	if (m_PokemonInfo.iLv >= 7)
		m_PokemonInfo.bEvolution = true;
	m_PokemonInfo.bLvUp = false;
}
void CCaterpie::Reset_Battle()
{
	m_iAnim = 0;
	m_bSetPos = false;
	m_bBrath = false;
...
HRESULT CCaterpie::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CCaterpie * CCaterpie::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CCaterpie*	pInstance = new CCaterpie(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CCaterpie"));
...
CGameObject * CCaterpie::Clone(void * pArg)
{
	CGameObject*	pInstance = new CCaterpie(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CCaterpie"));
		Safe_Release(pInstance);
	}
