...
CTr0::CTr0(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr0::CTr0(const CTr0 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr0::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr0::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr0::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr0::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr0::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr0::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr0"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr0::Move(_float fTimeDelta)
{
...
void CTr0::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("안녕?!"));
	m_vNormalScript.push_back(TEXT("내가 배지를 가진 관장이야!"));
	m_vNormalScript.push_back(TEXT("난 무척 강하다고 그래도 싸우겠어?\n그래? 어쩔 수 없겠네..."));
	

	wstring szScriptBegin = TEXT("관장의 힘을 보여주지!!\n가라!!     '");
...
void CTr0::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr0::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(1000);
...
void CTr0::Battle(_float fTimeDelta)
{
...
HRESULT CTr0::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr0 * CTr0::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr0*	pInstance = new CTr0(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr0"));
...
CGameObject * CTr0::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr0(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr0"));
		Safe_Release(pInstance);
	}
