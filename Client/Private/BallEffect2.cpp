#include "stdafx.h"
#include "..\Public\BallEffect2.h"

#include "GameInstance.h"
#include "Level_GamePlay.h"

CBallEffect2::CBallEffect2(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CBallEffect2::CBallEffect2(const CBallEffect2 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CBallEffect2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBallEffect2::Initialize(void * pArg)
{
...
void CBallEffect2::Tick(_float fTimeDelta)
{
	Set_Pos(fTimeDelta);

}

void CBallEffect2::Late_Tick(_float fTimeDelta)
{
	if (!g_PokeInfo && !g_bPokeDeck && nullptr != m_pRendererCom)
		m_pRendererCom->Add_RenderGroup(CRenderer::RENDER_ALPHABLEND, this);
}

HRESULT CBallEffect2::Render()
{
...
HRESULT CBallEffect2::Render_ShadowDepth()
{
	return E_NOTIMPL;
}
HRESULT CBallEffect2::Ready_Components()
{
	/* For.Com_Renderer */
...
void CBallEffect2::Set_Pos(_float fTimeDelta)
{
	_vector vPos = m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	_vector vUp = m_pTransformCom->Get_State(CTransform::STATE_UP);

	vPos += XMVector3Normalize(vUp) * 10.f * fTimeDelta;
...
HRESULT CBallEffect2::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CBallEffect2 * CBallEffect2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBallEffect2*	pInstance = new CBallEffect2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CBallEffect2"));
...
CGameObject * CBallEffect2::Clone(void * pArg)
{
	CGameObject*	pInstance = new CBallEffect2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBallEffect2"));
		Safe_Release(pInstance);
	}
