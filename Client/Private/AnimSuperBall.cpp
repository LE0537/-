#include "stdafx.h"
#include "..\Public\AnimSuperBall.h"
...
CAnimSuperBall::CAnimSuperBall(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CAnimSuperBall::CAnimSuperBall(const CAnimSuperBall & rhs)
	: CGameObj(rhs)
{
}

HRESULT CAnimSuperBall::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CAnimSuperBall::Initialize(void * pArg)
{
...
void CAnimSuperBall::Tick(_float fTimeDelta)
{
...
void CAnimSuperBall::Late_Tick(_float fTimeDelta)
{
...
HRESULT CAnimSuperBall::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CAnimSuperBall::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("AnimSuperBall"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CAnimSuperBall::Move(_float fTimeDelta)
{
...
void CAnimSuperBall::Ready_SuccessScript()
{
...
void CAnimSuperBall::Ready_SuccessFailScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
...
void CAnimSuperBall::Ready_FailScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
...
void CAnimSuperBall::Capture_Success()
{
	for (_int i = 0; i < 6; ++i)
	{
...
HRESULT CAnimSuperBall::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CAnimSuperBall * CAnimSuperBall::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CAnimSuperBall*	pInstance = new CAnimSuperBall(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CAnimSuperBall"));
...
CGameObject * CAnimSuperBall::Clone(void * pArg)
{
	CGameObject*	pInstance = new CAnimSuperBall(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CAnimSuperBall"));
		Safe_Release(pInstance);
	}
