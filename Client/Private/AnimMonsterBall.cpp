#include "stdafx.h"
#include "..\Public\AnimMonsterBall.h"
...
CAnimMonsterBall::CAnimMonsterBall(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CAnimMonsterBall::CAnimMonsterBall(const CAnimMonsterBall & rhs)
	: CGameObj(rhs)
{
}

HRESULT CAnimMonsterBall::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CAnimMonsterBall::Initialize(void * pArg)
{
...
void CAnimMonsterBall::Tick(_float fTimeDelta)
{
	Move(fTimeDelta);
...
void CAnimMonsterBall::Late_Tick(_float fTimeDelta)
{
...
HRESULT CAnimMonsterBall::Render()
{
...
HRESULT CAnimMonsterBall::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CAnimMonsterBall::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("AnimMonsterBall"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CAnimMonsterBall::Move(_float fTimeDelta)
{
	...
	if (m_bCapture && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		else
		{
			...
			CSoundMgr::Get_Instance()->PlayEffect(TEXT("Ball3.mp3"), 0.7f);
		}
		m_pModelCom->Set_CurrentAnimIndex(m_iAnimIndex);
...
void CAnimMonsterBall::Ready_SuccessScript()
{
...
void CAnimMonsterBall::Ready_SuccessFailScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
...
void CAnimMonsterBall::Ready_FailScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
...
void CAnimMonsterBall::Capture_Success()
{
	for (_int i = 0; i < 6; ++i)
	{
...
_bool CAnimMonsterBall::Capture()
{
...
HRESULT CAnimMonsterBall::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CAnimMonsterBall * CAnimMonsterBall::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CAnimMonsterBall*	pInstance = new CAnimMonsterBall(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CAnimMonsterBall"));
...
CGameObject * CAnimMonsterBall::Clone(void * pArg)
{
	CGameObject*	pInstance = new CAnimMonsterBall(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CAnimMonsterBall"));
		Safe_Release(pInstance);
	}
