...
HRESULT CGyarados::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Bite"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_HydroPump"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
...
HRESULT CGyarados::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CGyarados::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
void CGyarados::Ready_ClearEvolScript()
{
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'갸라도스'는 하이드로펌프를 배웠다.\n'갸라도스'는 깨물어부수기를 배웠다.\n'갸라도스'는 용의파동을 배웠다."));
}
