...
CTr7::CTr7(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr7::CTr7(const CTr7 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr7::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr7::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr7::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr7::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr7::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr7::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr7"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr7::Move(_float fTimeDelta)
{
...
void CTr7::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("어머!"));
	m_vNormalScript.push_back(TEXT("요즘 배지헌터가 있다던데 너 구나!"));
	m_vNormalScript.push_back(TEXT("가만두지 않겠어 오래노나카마를 울렸겠다!"));

	wstring szScriptBegin = TEXT("복수해주겠어!!\n가라!!     '");
...
void CTr7::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("나니...? 졌다고...?");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("얼음의배지를 얻었습니다.\n 6000원을 얻었다."));
}
void CTr7::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr7::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr7::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(6000);
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_IceBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr7::Battle(_float fTimeDelta)
{
...
HRESULT CTr7::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr7 * CTr7::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr7*	pInstance = new CTr7(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr7"));
...
CGameObject * CTr7::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr7(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr7"));
		Safe_Release(pInstance);
	}
