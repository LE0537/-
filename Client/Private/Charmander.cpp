#include "stdafx.h"
#include "..\Public\Charmander.h"
#include "GameInstance.h"


CCharmander::CCharmander(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CCharmander::CCharmander(const CCharmander & rhs)
	: CGameObj(rhs)
{
}

HRESULT CCharmander::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CCharmander::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	m_PokemonInfo.strName = TEXT("ÆÄÀÌ¸®");
	m_PokemonInfo.strInfo = TEXT("Å·°«Á¦³×·²Àº¸Ï.\n¾ÆÇªÁö¸»°í ÄÚ·Î³ª »¡¸® ³´ÀÚ ¤Ì¤Ì\n³»³â¿¡ °£È£»çÀº¸Ï °¡º¸ÀÚ°í");
	m_PokemonInfo.strChar = TEXT("±×Àú½Å");
	m_PokemonInfo.iPokeNum = 4;
...
void CCharmander::Tick(_float fTimeDelta)
{
	if (m_bOnOff)
	{
		Set_DeckPos();
...
void CCharmander::Late_Tick(_float fTimeDelta)
{
...
HRESULT CCharmander::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Prototype_Component_Model_Charmander"), (CComponent**)&m_pModelCom)))
		return E_FAIL;

	return S_OK;
}
void CCharmander::Set_DeckPos()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CCharmander::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CCharmander::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CCharmander * CCharmander::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CCharmander*	pInstance = new CCharmander(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CCharmander"));
...
CGameObject * CCharmander::Clone(void * pArg)
{
	CGameObject*	pInstance = new CCharmander(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CCharmander"));
		Safe_Release(pInstance);
	}
