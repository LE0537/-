...
HRESULT CDragonite::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_DragonPulse"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_MegaPunch"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum3)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BodyPress"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum4)))
		return E_FAIL;
...
HRESULT CDragonite::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CDragonite::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
void CDragonite::Ready_ClearEvolScript()
{
	...
	wstring strTextBegin = TEXT("ÃàÇÏÇÕ´Ï´Ù! ½Å´¨Àº  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'¸Á³ª´¨'Àº ¸Þ°¡ÅæÆÝÄ¡¸¦ ¹è¿ü´Ù.\n'¸Á³ª´¨'Àº ¹ÙµðÇÁ·¹½º¸¦ ¹è¿ü´Ù."));
}
...
void CDragonite::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("¸Á³ª´¨");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("¸Á³ª´¨");
	m_PokemonInfo.strInfo = TEXT("¹Ù´ÙÀÇ È­½ÅÀÌ¶ó°í ºÒ¸°´Ù.\n¸Á³ª´¨ ÇüÅÂÀÇ Á¶°¢»óÀ» ¹î¸Ó¸®¿¡ µÎ´Â ¹èµµ ¸¹´Ù.");
	m_PokemonInfo.strChar = TEXT("È­½Å");
