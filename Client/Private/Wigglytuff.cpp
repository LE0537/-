...
HRESULT CWigglytuff::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BodyPress"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
...
HRESULT CWigglytuff::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CWigglytuff::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
void CWigglytuff::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CWigglytuff::Move(_float fTimeDelta)
{
...
void CWigglytuff::Ready_EvolScript()
{
...
void CWigglytuff::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 푸린은  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'푸크린'은 바디프레스를 배웠다.\n'푸크린'은 메가톤펀치를 배웠다."));
	
}
void CWigglytuff::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CWigglytuff::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("푸크린");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("푸크린");
	m_PokemonInfo.strInfo = TEXT("킹갓지우!!");
