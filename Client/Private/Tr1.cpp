...
CTr1::CTr1(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr1::CTr1(const CTr1 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr1::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr1::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr1::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr1::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr1::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr1::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr1"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr1::Move(_float fTimeDelta)
{
...
void CTr1::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("어이~배지를 모으고 있지?"));
	m_vNormalScript.push_back(TEXT("난 물을 좋아해서 물배지를 맡고 있지!"));
	m_vNormalScript.push_back(TEXT("치사하게 전기타입을 쓰는건 아니겠지?"));

	wstring szScriptBegin = TEXT("배지를 원하나? 날 이겨봐!!\n가라!!     '");
...
void CTr1::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("너 강하구나...배지를 줄게...");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("물의배지를 얻었습니다.\n 2000원을 얻었다."));
}
void CTr1::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr1::Check_Anim(_float fTimeDelta)
{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_WaterBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr1::Battle(_float fTimeDelta)
{
...
HRESULT CTr1::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr1 * CTr1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr1*	pInstance = new CTr1(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr1"));
...
CGameObject * CTr1::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr1(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr1"));
		Safe_Release(pInstance);
	}
