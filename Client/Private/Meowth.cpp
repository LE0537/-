...
CMeowth::CMeowth(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CMeowth::CMeowth(const CMeowth & rhs)
	: CGameObj(rhs)
{
}

HRESULT CMeowth::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMeowth::Initialize(void * pArg)
{
...
void CMeowth::Tick(_float fTimeDelta)
{
	//CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);
	...
	//	pData_Manager->TCtoC(TEXT("Jigglypuff"), cName);
	//	pData_Manager->Conv_Bin_Model(m_pModelCom, cName, CData_Manager::DATA_ANIM);
	//	ERR_MSG(TEXT("Save_Bin_Jigglypuff"));
...
void CMeowth::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMeowth::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Meowth"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CMeowth::Move(_float fTimeDelta)
{
...
void CMeowth::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
	...
	if (m_bEvol && m_EvolTime > m_fRenderTime)
	{
		if (m_bRender)
		{
			...
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CPersian*>(m_EvolPoke)->Set_Render(true);
		}
		else if (!m_bRender)
		{
			m_bRender = true;
			m_EvolTime = 0.f;
			m_fRenderTime -= 0.05f;
			dynamic_cast<CPersian*>(m_EvolPoke)->Set_Render(false);
		}
	}
	if (m_fRenderTime < 0.f)
	{
		...
		Set_Dead();
		dynamic_cast<CPersian*>(m_EvolPoke)->Set_Render(true);
		dynamic_cast<CPersian*>(m_EvolPoke)->Set_CheckEvol();
	}
	if (!m_bSetPos)
	{
		_vector		vLook = { -0.3f,0.f,-1.f,0.f };
	...
	if (m_bSetPos && m_iAnimIndex == 1 && m_pModelCom->Get_End(m_iAnimIndex))
	{
		...
		if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Persian"), LEVEL_STATIC, TEXT("Layer_Pokemon"), &m_EvolPoke)))
			return;
...
void CMeowth::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMeowth::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.085f, 0.085f, 0.085f, 0.f));
...
void CMeowth::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("나옹");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("나옹");
	m_PokemonInfo.strInfo = TEXT("가보자고가보자고가보자고가보자고\n가보자고가보자고가보자고가보자고");
	m_PokemonInfo.strChar = TEXT("가보자고");
...
HRESULT CMeowth::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMeowth * CMeowth::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMeowth*	pInstance = new CMeowth(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMeowth"));
...
CGameObject * CMeowth::Clone(void * pArg)
{
	CGameObject*	pInstance = new CMeowth(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMeowth"));
		Safe_Release(pInstance);
	}
