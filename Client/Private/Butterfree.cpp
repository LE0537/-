...
CButterfree::CButterfree(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CButterfree::CButterfree(const CButterfree & rhs)
	: CGameObj(rhs)
{
}

HRESULT CButterfree::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CButterfree::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Tackle"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BugBuzz"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
...
void CButterfree::Tick(_float fTimeDelta)
{
...
void CButterfree::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CButterfree::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CButterfree::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CButterfree::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Butterfree"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CButterfree::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CButterfree::Move(_float fTimeDelta)
{
...
void CButterfree::Ready_EvolScript()
{
...
void CButterfree::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 단데기는  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'버터플'은 벌레의 야단법석을 배웠다."));
}
void CButterfree::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CButterfree::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CButterfree::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
...
void CButterfree::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("버터플");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("버터플");
	m_PokemonInfo.strInfo = TEXT("매일 꿀을 모으러 다닌다. \n다리의 솜털에 꿀을 발라 둥지로 가지고 가는 습성이 있다.");
	m_PokemonInfo.strChar = TEXT("신중");
	m_PokemonInfo.iPokeNum = 12;
	m_PokemonInfo.iLv = rand() % 10 + 10;
...
HRESULT CButterfree::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CButterfree * CButterfree::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CButterfree*	pInstance = new CButterfree(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CButterfree"));
...
CGameObject * CButterfree::Clone(void * pArg)
{
	CGameObject*	pInstance = new CButterfree(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CButterfree"));
		Safe_Release(pInstance);
	}
