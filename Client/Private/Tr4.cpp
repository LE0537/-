...
CTr4::CTr4(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr4::CTr4(const CTr4 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr4::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr4::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr4::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr4::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr4::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr4"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr4::Move(_float fTimeDelta)
{
...
void CTr4::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("흐규규규규규규"));
	m_vNormalScript.push_back(TEXT("흐흐흐ㅡㄷ드드그구그그규규"));
	m_vNormalScript.push_back(TEXT("흐끍흐륶끍!!!!!!!"));

	wstring szScriptBegin = TEXT("!!!!!!@#%@#%@!!\n가라!!     '");
...
void CTr4::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("민즙민즙 ㅠㅠ...");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("유령의배지를 얻었습니다.\n 4000원을 얻었다."));
}
void CTr4::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr4::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr4::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(4000);
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_GhostBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr4::Battle(_float fTimeDelta)
{
...
HRESULT CTr4::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr4 * CTr4::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr4*	pInstance = new CTr4(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr4"));
...
CGameObject * CTr4::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr4(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr4"));
		Safe_Release(pInstance);
	}
