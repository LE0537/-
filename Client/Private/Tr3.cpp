...
CTr3::CTr3(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr3::CTr3(const CTr3 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr3::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr3::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr3::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr3::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr3::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr3::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr3"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr3::Move(_float fTimeDelta)
{
...
void CTr3::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("오! 너 좀 강해 보이는데?"));
	m_vNormalScript.push_back(TEXT("나랑 승부 해보지 않을래?"));
	m_vNormalScript.push_back(TEXT("이래뵈도 나 관장이야!\n날 이기면 배지를 주지!"));

	wstring szScriptBegin = TEXT("기합으로 간다!!하~~~압!\n가라!!     '");
...
void CTr3::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("단련...이 부족했나...");
	m_vBattleScript.push_back(strTextBegin);
	m_vBattleScript.push_back(TEXT("격투의배지를 얻었습니다.\n 3500원을 얻었다."));
}
void CTr3::OnNavi()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CTr3::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr3::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(3500);
			...
			for (_int i = 0; i < 60; ++i)
			{
				if (dynamic_cast<CPlayer*>(m_pTarget)->Get_Bag()->Get_vecItemNum(i) == 99)
				{
					...
					ITEMINFO* tInfo;
					if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_FightBadge"), LEVEL_STATIC, TEXT("Layer_Item"), &tInfo)))
						return;
...
void CTr3::Battle(_float fTimeDelta)
{
...
HRESULT CTr3::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr3 * CTr3::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr3*	pInstance = new CTr3(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr3"));
...
CGameObject * CTr3::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr3(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr3"));
		Safe_Release(pInstance);
	}
