...
CChansey::CChansey(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CChansey::CChansey(const CChansey & rhs)
	: CGameObj(rhs)
{
}

HRESULT CChansey::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CChansey::Initialize(void * pArg)
{
...
void CChansey::Tick(_float fTimeDelta)
{
...
void CChansey::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CChansey::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CChansey::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CChansey::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Chansey"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CChansey::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CChansey::Move(_float fTimeDelta)
{
...
void CChansey::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CChansey::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.065f, 0.065f, 0.065f, 0.f));
...
void CChansey::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("럭키");
	m_PlayerInfo.bEvent = false;

	m_PokemonInfo.strName = TEXT("럭키");
	m_PokemonInfo.strInfo = TEXT("원래 움직임이 둔했지만 \n알을 노리는 자들로부터 지켜내기 위해 도주 속도가 빨라졌다.");
	m_PokemonInfo.strChar = TEXT("쏘큐트");
	m_PokemonInfo.iPokeNum = 113;
...
HRESULT CChansey::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CChansey * CChansey::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CChansey*	pInstance = new CChansey(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CChansey"));
...
CGameObject * CChansey::Clone(void * pArg)
{
	CGameObject*	pInstance = new CChansey(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CChansey"));
		Safe_Release(pInstance);
	}
