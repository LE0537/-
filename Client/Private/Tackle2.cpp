...
CTackle2::CTackle2(const CTackle2 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTackle2::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTackle2::Initialize(void * pArg)
{
...
void CTackle2::Tick(_float fTimeDelta)
{
	Set_Pos(fTimeDelta);

}

void CTackle2::Late_Tick(_float fTimeDelta)
{
...
HRESULT CTackle2::Ready_Components()
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_Tackle"), (CComponent**)&m_pTextureCom)))
		return E_FAIL;
...
void CTackle2::Set_Pos(_float fTimeDelta)
{
	m_fSize += 12.f * fTimeDelta;
...
HRESULT CTackle2::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTackle2 * CTackle2::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTackle2*	pInstance = new CTackle2(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTackle2"));
...
CGameObject * CTackle2::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTackle2(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTackle2"));
		Safe_Release(pInstance);
	}
