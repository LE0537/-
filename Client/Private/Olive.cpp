...
#include "Player.h"
#include "Ball.h"
#include "Data_Manager.h"	// √ﬂ∞°
#include "VIBuffer_Navigation.h"
#include "Bag.h"

COlive::COlive(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

COlive::COlive(const COlive & rhs)
	: CGameObj(rhs)
{
}

HRESULT COlive::Initialize_Prototype()
{
	return S_OK;
}

HRESULT COlive::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void COlive::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void COlive::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT COlive::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT COlive::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Olive"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void COlive::Move(_float fTimeDelta)
{
...
void COlive::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("√£æ“¥Ÿ!!"));
	m_vNormalScript.push_back(TEXT("¥‹µ®¥‘¿ª ≥Î∏Æ∞Ì ¿÷¥Ÿ¥¬ ≥‡ºÆ¿Ã ≥ ¡ˆ?"));
	m_vNormalScript.push_back(TEXT("¥‹µ®¥‘≤≤ ∞°∞‘ µŒ¡ˆ æ æ∆!!"));

	wstring szScriptBegin = TEXT("¥‹µ®¥‘¿∫ ≥™∏∏¿« ∞Õ¿Ã∂Û∞Ì!!\n∞°∂Û!!     '");
	wstring szScriptEnd = TEXT("'!!     ≥ ∑Œ ¡§«ﬂ¥Ÿ!");
...
void COlive::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("¥‹...µ®... »Â±‘»Â±‘ §–§–\n πŒ¡ÛπŒ¡Û §–§–");
...
void COlive::Ckeck_Dist()
{
	_vector vTargetPos = dynamic_cast<CGameObj*>(m_pTarget)->Get_Transfrom()->Get_State(CTransform::STATE_TRANSLATION);
	_vector vLook = vTargetPos - m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	m_fDist = XMVectorGetX(XMVector3Length(vLook));
}
void COlive::Check_Coll()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void COlive::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void COlive::Check_Anim(_float fTimeDelta)
{
...
void COlive::Battle(_float fTimeDelta)
{
...
HRESULT COlive::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
COlive * COlive::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	COlive*	pInstance = new COlive(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : COlive"));
...
CGameObject * COlive::Clone(void * pArg)
{
	CGameObject*	pInstance = new COlive(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : COlive"));
		Safe_Release(pInstance);
	}
