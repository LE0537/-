...
CBeat::CBeat(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CBeat::CBeat(const CBeat & rhs)
	: CGameObj(rhs)
{
}

HRESULT CBeat::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CBeat::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CBeat::Tick(_float fTimeDelta)
{
...
void CBeat::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBeat::Ready_Components()
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Beat"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CBeat::Move(_float fTimeDelta)
{
...
void CBeat::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("어이!!"));
	m_vNormalScript.push_back(TEXT("너도 체육관 배지를 모으는 중이지?"));
	m_vNormalScript.push_back(TEXT("난 이미 3개나 모았다고 나랑 승부하자!"));
	m_vNormalScript.push_back(TEXT("설마 도망치진 않겠지?"));

	wstring szScriptBegin = TEXT("후훗 간만에 불타오르는걸!!\n가라!!     '");
...
void CBeat::Ready_LoseText()
{
	...
	wstring strTextBegin = TEXT("ㄷㄷ;;뭐야...너 강하자나...");
...
void CBeat::Ckeck_Dist()
{
	_vector vTargetPos = dynamic_cast<CGameObj*>(m_pTarget)->Get_Transfrom()->Get_State(CTransform::STATE_TRANSLATION);
	_vector vLook = vTargetPos - m_pTransformCom->Get_State(CTransform::STATE_TRANSLATION);
	m_fDist = XMVectorGetX(XMVector3Length(vLook));
}
void CBeat::Check_Coll()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CBeat::BattleStart(_float fTimeDelta)
{
	...
	if (!m_bBattle)
	{
		...
		if ((m_fStartBattle > 0.2f) && m_pModelCom->Get_End(1))
		{
			...
			dynamic_cast<CBall*>(m_pBall)->Set_Render(false, 1);
...
void CBeat::Check_Anim(_float fTimeDelta)
{
...
void CBeat::Battle(_float fTimeDelta)
{
...
HRESULT CBeat::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CBeat * CBeat::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CBeat*	pInstance = new CBeat(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CBeat"));
...
CGameObject * CBeat::Clone(void * pArg)
{
	CGameObject*	pInstance = new CBeat(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CBeat"));
		Safe_Release(pInstance);
	}
