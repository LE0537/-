...
CWartortle::CWartortle(const CWartortle & rhs)
	: CGameObj(rhs)
{
}

HRESULT CWartortle::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CWartortle::Initialize(void * pArg)
{
...
void CWartortle::Tick(_float fTimeDelta)
{
...
void CWartortle::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (!m_PokemonInfo.bRace)
	{
...
HRESULT CWartortle::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
void CWartortle::Race(_float fTimeDelta)
{
	if (m_iAnimIndex == 8)
	{
...
HRESULT CWartortle::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Wartortle"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CWartortle::Ready_WildBattle()
{
	m_vecPoke.reserve(6);;
...
void CWartortle::Move(_float fTimeDelta)
{
...
void CWartortle::Ready_EvolScript()
{
...
void CWartortle::Ready_ClearEvolScript()
{
	for (auto iter = m_vNormalScript.begin(); iter != m_vNormalScript.end();)
		iter = m_vNormalScript.erase(iter);
	...
	wstring strTextBegin = TEXT("축하합니다! 꼬부기는  \n'");
	...
	m_vNormalScript.push_back(strTextBegin);
	m_vNormalScript.push_back(TEXT("'어니부기'는 거품광선을 배웠다."));
}
void CWartortle::Set_EvolPos(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
	if (m_bSetPos)
	{
...
void CWartortle::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CWartortle::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.065f, 0.065f, 0.065f, 0.f));
...
void CWartortle::Set_Stats()
{
	...
	m_PlayerInfo.strName = TEXT("어니부기");
	m_PlayerInfo.bEvent = false;


	m_PokemonInfo.strName = TEXT("어니부기");
	m_PokemonInfo.strInfo = TEXT("탐스러운 귀와 꼬리를 능숙하게 다뤄서 \n물속에서의 균형을 유지한다.");
	m_PokemonInfo.strChar = TEXT("대장부");
	m_PokemonInfo.iPokeNum = 8;
...
HRESULT CWartortle::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CWartortle * CWartortle::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CWartortle*	pInstance = new CWartortle(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CWartortle"));
...
CGameObject * CWartortle::Clone(void * pArg)
{
	CGameObject*	pInstance = new CWartortle(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CWartortle"));
		Safe_Release(pInstance);
	}
