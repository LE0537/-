...
CTr9::CTr9(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CTr9::CTr9(const CTr9 & rhs)
	: CGameObj(rhs)
{
}

HRESULT CTr9::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CTr9::Initialize(void * pArg)
{
	m_vecPoke.reserve(6);
...
void CTr9::Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
void CTr9::Late_Tick(_float fTimeDelta)
{
	if (!g_bRace)
	{
...
HRESULT CTr9::Render_ShadowDepth()
{
	if (nullptr == m_pShaderCom ||
		nullptr == m_pModelCom)
		return E_FAIL;
...
HRESULT CTr9::Ready_Components()
{
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Tr9"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CTr9::Move(_float fTimeDelta)
{
...
void CTr9::Ready_Script()
{
	m_vNormalScript.push_back(TEXT("안녕? 난 금랑!"));
	m_vNormalScript.push_back(TEXT("탐정이죠."));
	m_vNormalScript.push_back(TEXT("너가 배지를 노리고 있다는건 다 알고있다."));
	m_vNormalScript.push_back(TEXT("하지만 난 최강종 드래곤 배지를 가지고 있지!"));
	m_vNormalScript.push_back(TEXT("챔피온을 빼면 이세카이 최강은 바로 나다!"));

	wstring szScriptBegin = TEXT("최강종의 맛을 보여주지!!\n가라!!     '");
...
void CTr9::BattleStart(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		if (!m_bEffectEnd)
...
void CTr9::Check_Anim(_float fTimeDelta)
{
	if (m_iAnimIndex == 1 || m_iAnimIndex == 4)
	{
		...
		if (m_bLose && m_pModelCom->Get_End(m_iAnimIndex))
		{
			...
			dynamic_cast<CGameObj*>(m_pTarget)->Set_Money(7000);
...
void CTr9::Battle(_float fTimeDelta)
{
...
HRESULT CTr9::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CTr9 * CTr9::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CTr9*	pInstance = new CTr9(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CTr9"));
...
CGameObject * CTr9::Clone(void * pArg)
{
	CGameObject*	pInstance = new CTr9(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CTr9"));
		Safe_Release(pInstance);
	}
