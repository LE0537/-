...
CMewtwo::CMewtwo(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CGameObj(pDevice, pContext)
{
}

CMewtwo::CMewtwo(const CMewtwo & rhs)
	: CGameObj(rhs)
{
}

HRESULT CMewtwo::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CMewtwo::Initialize(void * pArg)
{
	...
	*(CGameObject**)pArg = this;
	...
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_BreakCar"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum1)))
		return E_FAIL;
	if (FAILED(pGameInstance->Add_GameObject(TEXT("Prototype_GameObject_Psychic"), LEVEL_STATIC, TEXT("Layer_Skill"), &m_PokemonInfo.eSkillNum2)))
		return E_FAIL;
...
void CMewtwo::Tick(_float fTimeDelta)
{
	if (m_bOnOff)
	{
		Set_DeckPos();
...
void CMewtwo::Late_Tick(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
HRESULT CMewtwo::Ready_Components()
{
	/* For.Com_Renderer */
	...
	if (FAILED(__super::Add_Components(TEXT("Com_ShaderUI"), LEVEL_STATIC, TEXT("Prototype_Component_Shader_VtxUIModel"), (CComponent**)&m_pShaderUICom)))
		return E_FAIL;
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Mewtwo"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
void CMewtwo::Move(_float fTimeDelta)
{
...
void CMewtwo::Key_Input(_float fTimeDelta)
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
void CMewtwo::Battle(_float fTimeDelta)
{
	if (!m_bBattle)
	{
		m_pTransformCom->Set_Scale(XMVectorSet(0.04f, 0.04f, 0.04f, 0.f));
...
HRESULT CMewtwo::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CMewtwo * CMewtwo::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CMewtwo*	pInstance = new CMewtwo(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CMewtwo"));
...
CGameObject * CMewtwo::Clone(void * pArg)
{
	CGameObject*	pInstance = new CMewtwo(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CMewtwo"));
		Safe_Release(pInstance);
	}
